...
# ggplot(dflongi,aes(times,ADL),size=2)+
  # facet_wrap( ~ outcomes)+geom_path(col = 1,group=dflongi$id)+geom_smooth(method="lm")+
  # xlab("Time in days after baseline")
# ggplot(dflongi,aes(times,emo),size=2)+
  # facet_wrap( ~ outcomes)+geom_path(col = 1,group=dflongi$id)+geom_smooth(method="lm")+
  # xlab("Time in days after baseline")
...
# dflongiADL <- subset(dflongi,times!="NA" & ADL!="NA")
# dflongiemo <- subset(dflongi,times!="NA" & emo!="NA")
# lmeFit <- lme(ADL~times, data = dflongiADL, random = ~ times | id)
# lmeFit
# lmeFit2 <- lme(emo~times, data = dflongiemo, random = ~ times | id)
# lmeFit2

# dflongiU <- subset(dflongi,emo!="NA" & ADL!="NA")
# cor(dflongiU$emo,dflongiU$MSE)

# dflongiADL <- subset(dflongi,times!="NA" & ADL!="NA")
# dflongiADL<-dflongiADL[-which(dflongiADL$times>dflongiADL$totalTime),]
# p<-unique(dflongiADL$id)
# dfsurFor <- subset(dfsur,dfsur$marriage!="NA" & dfsur$sex !="NA" & dfsur$age !="NA" 
                   # & dfsur$residenc != "NA" & dfsur$edu !="NA" & dfsur$occ != "NA")
# q<-unique(dfsurFor$id)
# w<-intersect(p,q)
# dfsurADL <- dfsurADL[which(dfsurADL$id %in% w),]
# dflongiADL <- dflongiADL[which(dflongiADL$id %in% w),]
# lmeFit <- lme(ADL~times, data = dflongiADL, random = ~ times | id)
# lmeFit

# coxFit <- coxph(Surv(totalTime, event=outcome) ~ sex+age+residenc+edu+occ+marriage,
                # data = dfsurADL,x=TRUE)
# coxFit

# #jointFit <- jointModelBayes(lmeFit, coxFit, timeVar = "times",n.iter = 3000)

# jointFit <- jointModelBayes(lmeFit, coxFit, timeVar = "times",n.iter = 300)
# summary(jointFit)

# lmeFitns <- lme(ADL~ ns(times,2), data = dflongiADL, random = ~ ns(times,2) | id)
