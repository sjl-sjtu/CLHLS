...
'''
mods = "SWB_BMI_ADL"
cont_list = ['SWB','BMI','ADL']

if data_mode == 'PBC2':
    (x_dim, x_dim_cont, x_dim_bin), (data, time, label), (mask1, mask2, mask3), (data_mi) = impt.import_dataset(cont_list,norm_mode = 'standard')
    ...
    pred_time = [4,6,8,10] # prediction time (in days)
    eval_time = [2,5,7,10] # evaluation time (for C-index and Brier-Score)
else:
...
for tr_index, te_index in kf.split(data):
    tr_data,te_data, tr_data_mi, te_data_mi, tr_time,te_time, tr_label,te_label, tr_mask1,te_mask1, tr_mask2,te_mask2, tr_mask3,te_mask3 = data[tr_index],data[te_index], data_mi[tr_index],data_mi[te_index], time[tr_index], time[te_index], label[tr_index], label[te_index], mask1[tr_index], mask1[te_index],mask2[tr_index],mask2[te_index], mask3[tr_index],mask3[te_index]
...
final1 = reduce(lambda x, y: x + y, f1)
final2 = reduce(lambda x, y: x + y, f2)
final1 = final1/nfold
final2 = final2/nfold
...
df1.to_csv("C_index_"+mods+".csv")
df2.to_csv("BRIER-SCORE_"+mods+".csv")
