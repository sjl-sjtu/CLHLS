...
def import_dataset(cont_list,norm_mode = 'standard'):
    ...
    df_                = pd.read_csv("../dflongi_filtered.csv")  
    df_                = df_.dropna()
    df_["label"]       = df_["outcome"]
    df_["tte"]         = df_["totalTime"]
    df_["id"]          = df_["id"]
    df_["times"]       = df_["times"]
    bin_list           = ["sex","age","res","edu","occ","mar"]
    cont_list          = cont_list #['SWB']
