...
coxFit <- coxph(Surv(totalTime,event=outcome)~sex+res+edu+occ+mar,
                data=dfsur2,x=TRUE)
coxFit
...
summary(JMFit)$`CoefTable-Event` %>% 
  ...
  write_csv("SWB_assoc_younger_scale.csv")
print("#######")

#BMI
print("BMI")
dfsur1 <- dfsur%>%
               drop_na("sex","age","res","edu","occ","mar","totalTime")%>%filter(age %in% c(1,2))
...
summary(JMFit2)$`CoefTable-Event` %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "covariate") %>% 
  write_csv("BMI_assoc_younger_scale.csv")
print("#######")
...
summary(JMFit3)$`CoefTable-Event` %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "covariate") %>% 
  write_csv("ADL_assoc_younger_scale.csv")
...
dfsur1 <- dfsur%>%
               drop_na("sex","age","res","edu","occ","mar","totalTime")%>%filter(age %in% c(3,4))
dflongi2 <- dflongi%>%
                   drop_na("times","SWB")%>%
                   filter(times<=totalTime)%>%
                   filter(id %in% dfsur1$id)
...
summary(JMFit)$`CoefTable-Event` %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "covariate") %>% 
  write_csv("SWB_assoc_elder_scale.csv")
print("#######")
...
summary(JMFit2)$`CoefTable-Event` %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "covariate") %>% 
  write_csv("BMI_assoc_elder_scale.csv")
print("#######")
...
summary(JMFit3)$`CoefTable-Event` %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "covariate") %>% 
  write_csv("ADL_assoc_elder_scale.csv")
